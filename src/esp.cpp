...
INLINE box_t get_player_bounding_box(c_cs_player* player, esp_player_t& esp)
{
	...
	const vec3_t& origin = esp.origin;
	const vec3_t& mins = esp.mins;
	const vec3_t& maxs = esp.maxs;
	...
	for (int i = 0; i < 8; i++)
	{
		vec3_t point = origin + points[i];
		if (RENDER->world_to_screen(point, points_screen[i], false))
		{
			left = std::min(left, points_screen[i].x);
			top = std::min(top, points_screen[i].y);
			right = std::max(right, points_screen[i].x);
			bottom = std::max(bottom, points_screen[i].y);
			points_on_screen++;
		}
	}

	// If no points are on screen, the entity is not visible
	...
	// Validate box size (not too small or too large)
	if (width <= 1.f || height <= 1.f ||
		width > RENDER->screen.x * 2.f ||
		height > RENDER->screen.y * 2.f)
		return result;
	...
	// Check if completely offscreen (with some margin)
	const float margin = 10.f;
	result.offscreen = (right < -margin || left > RENDER->screen.x + margin ||
		bottom < -margin || top > RENDER->screen.y + margin);
	...
	LISTENER_ENTITY->for_each_player([&](c_cs_player* player)
		{
			...
			// Validate index bounds to prevent array overflow
			if (index < 0 || index >= 128)
				return;

			auto& esp = player_esp[index];

			// Skip teammates for enemy ESP
			...
			if (player->observer_mode() == OBS_MODE_IN_EYE || player->observer_mode() == OBS_MODE_CHASE)
			{
				...
				{
					auto observer_index = observer_target->index();
					if (observer_index >= 0 && observer_index < 128)
					{
						auto& observer_esp = player_esp[observer_index];
			...
			else
			{
				if (player->dormant())
				{
					...
					if (std::abs(dormant.time - HACKS->global_vars->curtime) > 15.f)
					{
						if (esp.alpha > 0.f)
							LERP_ALPHA(0.f);
					}
					...
					// Use more realistic player bounds
					if (dormant.maxs.valid() && dormant.mins.valid())
					{
					...
					{
						// Default player bounding box (standing)
...

			{
				if (esp.alpha > 0.5f && esp.health > 0)
				{
					...
					if (entity_visuals.elements & 64)
					{
						...
						if (entity_visuals.elements & esp_element_fake_duck && esp.fake_ducking)
							EMPLACE_OBJECT(true, FONT_OUTLINE | FONT_LIGHT_BACK, FONT_PIXEL, ESP_POS_RIGHT, 0.f, 0.f, esp.alpha, { 255, 210, 80, 255 }, { 0, 0, 0, 255 }, XOR("FD"));
