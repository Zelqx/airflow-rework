...
namespace hooks
{
	...
	LRESULT WINAPI hooked_wnd_proc(HWND wnd, uint32_t msg, WPARAM wp, LPARAM lp)
	{
		...
		// If the game window is not focused, forward message and clear chat state to be safe
		if (WINCALL(GetForegroundWindow)() != HACKS->window)
		{
			HACKS->open_chat = false;
			return CallWindowProcA(old_wndproc, wnd, msg, wp, lp);
		}

		// --- Chat open/close detection (do this BEFORE we update key states) ---
		// Default CS:GO chat keys: 'Y' (all) and 'U' (team). Close on Enter or Escape.
		if (msg == WM_KEYDOWN)
		{
			if (wp == 'Y' || wp == 'U')
				HACKS->open_chat = true;
			else if (wp == VK_RETURN || wp == VK_ESCAPE)
				HACKS->open_chat = false;
		}
		else if (msg == WM_CHAR)
		{
			// Some input backends send WM_CHAR for chat input; if Enter is received here, clear chat.
			if (wp == VK_RETURN || wp == VK_ESCAPE)
				HACKS->open_chat = false;
		}
		else if (msg == WM_KILLFOCUS)
		{
			HACKS->open_chat = false; // avoid getting stuck if window loses focus
		}

		// Update key states; key toggling will be skipped if open_chat is true
