...
std::array< std::string, max_tabs > tabs = {
	XOR("Rage"), XOR("Anti-hit"),
	XOR("Visuals"), XOR("Misc"), XOR("Skins"), XOR("Configs")
};
...
void c_menu::init_textures()
{
	...
	if (!logo_texture) {
    hr = add_texture_to_memory(RENDER->get_device(), ryzeLogo, sizeof(ryzeLogo), &logo_texture);
		if (FAILED(hr) || !logo_texture)
			OutputDebugStringA("[menu] Failed to load logo_texture\n");
		else
			OutputDebugStringA("[menu] logo_texture loaded successfully\n");
	}

	if (!keyboard_texture) {
		hr = add_texture_to_memory(RENDER->get_device(), keyboard_icon, sizeof(keyboard_icon), &keyboard_texture);
		if (FAILED(hr) || !keyboard_texture)
			OutputDebugStringA("[menu] Failed to load keyboard_texture\n");
		else
			OutputDebugStringA("[menu] keyboard_texture loaded successfully\n");
	}

	if (!warning_texture) {
		hr = add_texture_to_memory(RENDER->get_device(), warning_icon, sizeof(warning_icon), &warning_texture);
		if (FAILED(hr) || !warning_texture)
			OutputDebugStringA("[menu] Failed to load warning_texture\n");
		else
			OutputDebugStringA("[menu] warning_texture loaded successfully\n");
	}

	if (!spectator_texture) {
		hr = add_texture_to_memory(RENDER->get_device(), spectators_icon, sizeof(spectators_icon), &spectator_texture);
		if (FAILED(hr) || !spectator_texture)
			OutputDebugStringA("[menu] Failed to load spectator_texture\n");
		else
			OutputDebugStringA("[menu] spectator_texture loaded successfully\n");
	}

	if (!bomb_texture) {
		hr = add_texture_to_memory(RENDER->get_device(), bomb_indicator, sizeof(bomb_indicator), &bomb_texture);
		if (FAILED(hr) || !bomb_texture)
			OutputDebugStringA("[menu] Failed to load bomb_texture\n");
		else
			OutputDebugStringA("[menu] bomb_texture loaded successfully\n");
	}

	// Tab icons in order: Rage, Legit, Anti-Aim, Defensive, Visuals, Misc, Skins, Configs
	if (!icon_textures[0]) {
		hr = add_texture_to_memory(RENDER->get_device(), rage_icon, sizeof(rage_icon), &icon_textures[0]);
		if (FAILED(hr) || !icon_textures[0])
			OutputDebugStringA("[menu] Failed to load rage_icon\n");
		else
			OutputDebugStringA("[menu] rage_icon loaded\n");
	}

	if (!icon_textures[1]) {
		hr = add_texture_to_memory(RENDER->get_device(), legit_icon, sizeof(legit_icon), &icon_textures[1]);
		if (FAILED(hr) || !icon_textures[1])
			OutputDebugStringA("[menu] Failed to load legit_icon\n");
		else
			OutputDebugStringA("[menu] legit_icon loaded\n");
	}

	if (!icon_textures[2]) {
		hr = add_texture_to_memory(RENDER->get_device(), antihit_icon, sizeof(antihit_icon), &icon_textures[2]);
		if (FAILED(hr) || !icon_textures[2])
			OutputDebugStringA("[menu] Failed to load antihit_icon\n");
		else
			OutputDebugStringA("[menu] antihit_icon loaded\n");
	}

	if (!icon_textures[3]) {
		hr = add_texture_to_memory(RENDER->get_device(), warning_icon, sizeof(warning_icon), &icon_textures[3]); // Defensive
		if (FAILED(hr) || !icon_textures[3])
			OutputDebugStringA("[menu] Failed to load defensive_icon (warning)\n");
		else
			OutputDebugStringA("[menu] defensive_icon loaded\n");
	}

	if (!icon_textures[4]) {
		hr = add_texture_to_memory(RENDER->get_device(), visuals_icon, sizeof(visuals_icon), &icon_textures[4]);
		if (FAILED(hr) || !icon_textures[4])
			OutputDebugStringA("[menu] Failed to load visuals_icon\n");
		else
			OutputDebugStringA("[menu] visuals_icon loaded\n");
	}

	if (!icon_textures[5]) {
		hr = add_texture_to_memory(RENDER->get_device(), misc_icon, sizeof(misc_icon), &icon_textures[5]);
		if (FAILED(hr) || !icon_textures[5])
			OutputDebugStringA("[menu] Failed to load misc_icon\n");
		else
			OutputDebugStringA("[menu] misc_icon loaded\n");
	}

	if (!icon_textures[6]) {
		hr = add_texture_to_memory(RENDER->get_device(), skins_icon, sizeof(skins_icon), &icon_textures[6]);
		if (FAILED(hr) || !icon_textures[6])
			OutputDebugStringA("[menu] Failed to load skins_icon\n");
		else
			OutputDebugStringA("[menu] skins_icon loaded\n");
	}

	if (!icon_textures[7]) {
		hr = add_texture_to_memory(RENDER->get_device(), cfg_icon, sizeof(cfg_icon), &icon_textures[7]);
		if (FAILED(hr) || !icon_textures[7])
			OutputDebugStringA("[menu] Failed to load cfg_icon\n");
		else
			OutputDebugStringA("[menu] cfg_icon loaded\n");
	}

	// Avatar (if available)
	//if (!avatar && !HACKS->cheat_info.user_avatar.empty() && HACKS->cheat_info.user_avatar.size()) {
		//hr = add_texture_to_memory(RENDER->get_device(), HACKS->cheat_info.user_avatar.data(), HACKS->cheat_info.user_avatar.size(), &avatar);
		//if (FAILED(hr) || !avatar)
			//OutputDebugStringA("[menu] Failed to load avatar\n");
		//else
			//OutputDebugStringA("[menu] avatar loaded\n");
	//}
}
...
{
	// Guard: if a menu widget left the ImGui window/stack state bad (or Begin
	// failed), GetCurrentWindow() is null and ImGui::PopItemWidth() dereferences
	// it - this is the recurring 0xC0000005 crash (fault in ImGui::PopItemWidth,
	// called from window_end). Skip the teardown instead of crashing; the next
	// frame re-establishes everything.
	ImGuiContext* ctx = ImGui::GetCurrentContext();
	if (!ctx || !ctx->CurrentWindow)
		return;

	// Defense in depth: if any widget leaves the item-width stack empty,
	// PopItemWidth() underflows the size and reads past the (null) data
	// pointer. Skip the teardown rather than crash; next frame rebuilds.
	if (ctx->CurrentWindow->DC.ItemWidthStack.Size <= 0)
		return;
...
void c_menu::draw_ui_background()
{
	...
	if (logo_texture) {
		list->AddImage(
			...
			clr.new_alpha(window_alpha).as_imcolor()
		);
	}
...
void c_menu::draw_sub_tabs(int& selector, const std::vector< std::string >& tabs)
{
	...
	// Clip rectangles are NOT pushed here anymore.
	// They will be handled in draw_ui_items().
}
...
void c_menu::draw()
{
	...
	// If screen size is invalid, reset positions and exit
	...
	// --- Draw all indicators (with proper guards) ---
	OutputDebugStringA("[menu] drawing binds\n");
	this->draw_binds();

	OutputDebugStringA("[menu] drawing bomb indicator\n");
	this->draw_bomb_indicator();

	OutputDebugStringA("[menu] drawing watermark\n");
	this->draw_watermark();

	OutputDebugStringA("[menu] drawing spectators\n");
	this->draw_spectators();

	// --- AA State indicator – only if everything is ready ---
	//OutputDebugStringA("[menu] drawing AA state\n");
	//if (HACKS && HACKS->in_game && HACKS->local && HACKS->local->is_alive()) {
	//	this->draw_aa_state();
	//}
	//else {
	//	OutputDebugStringA("[menu] Skipping draw_aa_state: not fully ready\n");
	//}
