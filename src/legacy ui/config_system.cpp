...
inline void load_bind(json& j, std::string name, key_binds_t& v)
{
	...
	v.toggled = j[name][CXOR("toggled")].asBool();
}
...
namespace config
{
	...
	void create_config_folder()
	{
		...
		// std::string trash_talk_folder = CXOR("airflow\\Trash Talk");
		//
		// All data lives in Documents\Ryze now; migrate the old folder once.
		ryze_paths::ensure_folders();
		ryze_paths::migrate_old_data();

		folder = ryze_paths::config_folder();
		sounds_folder = ryze_paths::sounds_folder();
		std::string trash_talk_folder = ryze_paths::trash_talk_folder();
		...
		if (!is_dir(trash_talk_folder.c_str()))
			CreateDirectoryA(trash_talk_folder.c_str(), NULL);

		const std::string codex_trash_file = trash_talk_folder + CXOR("\\codex.txt");
		std::ifstream codex_file_check(codex_trash_file);
		if (!codex_file_check.good())
		{
			std::ofstream codex_file(codex_trash_file);
			if (codex_file.is_open())
			{
				codex_file
	...
	void create(const std::string& config_name)
	{
		...
		auto& rage = json_obj[CXOR("rage")];
		{
			...
			save_bool(rage, CXOR("multipoint_debug"), default_config.rage.multipoint_debug);
			save_bool(rage, CXOR("multipoint_advanced"), default_config.rage.multipoint_advanced);
			save_bool(rage, CXOR("debug_hitscan"), default_config.rage.debug_hitscan);
			save_bool(rage, CXOR("debug_log"), default_config.rage.debug_log);
			save_bool(rage, CXOR("backtrack"), default_config.rage.backtrack);
			save_int(rage, CXOR("backtrack_ms"), default_config.rage.backtrack_ms);
			save_bool(rage, CXOR("extrapolation"), default_config.rage.extrapolation);
			save_int(rage, CXOR("extrapolation_ticks"), default_config.rage.extrapolation_ticks);
			save_int(rage, CXOR("interpolation_ticks"), default_config.rage.interpolation_ticks);
			save_bool(rage, CXOR("baim_on_jitter"), default_config.rage.baim_on_jitter);
			save_bool(rage, CXOR("auto_delay"), default_config.rage.auto_delay);
			save_int(rage, CXOR("auto_delay_max"), default_config.rage.auto_delay_max);
			//save_int(rage, CXOR("defensive_mode"), default_config.rage.defensive_mode);
			...
			save_bool(rage, CXOR("delay_lc"), default_config.rage.delay_lc);
		...
		auto& antihit = json_obj[CXOR("antihit")];
		{
			...
			save_bool(antihit, CXOR("antiam"), default_config.antihit.antiam);
			save_int(antihit, CXOR("slow_walk_speed"), default_config.antihit.slow_walk_speed);
			save_bool(antihit, CXOR("at_targets"), default_config.antihit.at_targets);
			save_bool(antihit, CXOR("at_targets_enabled"), default_config.antihit.at_targets_enabled);
			save_int(antihit, CXOR("at_targets_offset"), default_config.antihit.at_targets_offset);
			save_int(antihit, CXOR("manual_offset"), default_config.antihit.manual_offset);
			...
			save_uint(antihit, CXOR("def_aa_conditions"), default_config.antihit.def_aa_conditions);
			save_int(antihit, CXOR("def_aa_phase_count"), default_config.antihit.def_aa_phase_count);
			for (int p = 0; p < 4; ++p)
			{
				auto phase_key = std::string("def_aa_phase_") + std::to_string(p);
				save_int(antihit[phase_key], CXOR("duration"), default_config.antihit.def_aa_phases[p].duration);
				save_int(antihit[phase_key], CXOR("yaw_mode"), default_config.antihit.def_aa_phases[p].yaw_mode);
				save_int(antihit[phase_key], CXOR("yaw_angle"), default_config.antihit.def_aa_phases[p].yaw_angle);
				save_int(antihit[phase_key], CXOR("pitch_mode"), default_config.antihit.def_aa_phases[p].pitch_mode);
				save_int(antihit[phase_key], CXOR("pitch_angle"), default_config.antihit.def_aa_phases[p].pitch_angle);
				save_int(antihit[phase_key], CXOR("pitch_angle2"), default_config.antihit.def_aa_phases[p].pitch_angle2);
				save_int(antihit[phase_key], CXOR("spin_rpm"), default_config.antihit.def_aa_phases[p].spin_rpm);
				save_int(antihit[phase_key], CXOR("yaw_rpm"), default_config.antihit.def_aa_phases[p].yaw_rpm);
				save_bool(antihit[phase_key], CXOR("spin_clockwise"), default_config.antihit.def_aa_phases[p].spin_clockwise);
				save_int(antihit[phase_key], CXOR("spin_inverter_mode"), default_config.antihit.def_aa_phases[p].spin_inverter_mode);
			}
			save_int(antihit, CXOR("def_aa_air_minimum_ticks"), g_cfg.antihit.def_aa_air_minimum_ticks);
			save_bool(antihit, CXOR("def_aa_enable"), default_config.antihit.def_aa_enable);
			save_bool(antihit, CXOR("def_aa_dt_active"), default_config.antihit.def_aa_dt_active);
			save_int(antihit, CXOR("def_aa_dt_mode"), default_config.antihit.def_aa_dt_mode);
			save_int(antihit, CXOR("def_aa_yaw_preset"), default_config.antihit.def_aa_yaw_preset);
			save_int(antihit, CXOR("def_aa_rotate_ticks"), default_config.antihit.def_aa_rotate_ticks);
			save_int(antihit, CXOR("def_aa_yaw_left"), default_config.antihit.def_aa_yaw_left);
			save_int(antihit, CXOR("def_aa_yaw_right"), default_config.antihit.def_aa_yaw_right);
			save_int(antihit, CXOR("def_aa_yaw_back"), default_config.antihit.def_aa_yaw_back);
			save_bool(antihit, CXOR("def_aa_random_yaw"), default_config.antihit.def_aa_random_yaw);
			save_int(antihit, CXOR("def_aa_random_yaw_min"), default_config.antihit.def_aa_random_yaw_min);
			save_int(antihit, CXOR("def_aa_random_yaw_max"), default_config.antihit.def_aa_random_yaw_max);
			save_int(antihit, CXOR("def_aa_pitch"), default_config.antihit.def_aa_pitch);
			save_bool(antihit, CXOR("def_aa_random_pitch"), default_config.antihit.def_aa_random_pitch);
			save_int(antihit, CXOR("def_aa_random_pitch_min"), default_config.antihit.def_aa_random_pitch_min);
			save_int(antihit, CXOR("def_aa_random_pitch_max"), default_config.antihit.def_aa_random_pitch_max);
			...
			save_bool(antihit, CXOR("peek_pitch_enable"), default_config.antihit.peek_pitch_enable);
			save_bool(antihit, CXOR("air_override"), default_config.antihit.air_override);
			save_bool(antihit, CXOR("velocity_yaw_bias"), default_config.antihit.velocity_yaw_bias);
			save_bool(antihit, CXOR("adaptive_jitter"), default_config.antihit.adaptive_jitter);
			save_bool(antihit, CXOR("landing_pitch"), default_config.antihit.landing_pitch);

			save_int(antihit, CXOR("pitch"), default_config.antihit.pitch);
			save_int(antihit, CXOR("yaw"), default_config.antihit.yaw);
			save_int(antihit, CXOR("custom_yaw"), default_config.antihit.custom_yaw);
			save_int(antihit, CXOR("ticks_defensive"), default_config.antihit.ticks_defensive);

			save_int(antihit, CXOR("def_aa_random_factor"), default_config.antihit.def_aa_random_factor);

			save_int(antihit, CXOR("yaw_add"), default_config.antihit.yaw_add);

			save_int(antihit, CXOR("spin_speed"), default_config.antihit.spin_speed);
			save_bool(antihit, CXOR("spin_clamp"), default_config.antihit.spin_clamp);
			...
			save_int(antihit, CXOR("peek_pitch_phase"), default_config.antihit.peek_pitch_phase);
			save_int(antihit, CXOR("peek_pitch_mode"), default_config.antihit.peek_pitch_mode);
			save_int(antihit, CXOR("peek_custom_pitch"), default_config.antihit.peek_custom_pitch);
			save_int(antihit, CXOR("peek_jitter_mode"), default_config.antihit.peek_jitter_mode);
			save_int(antihit, CXOR("peek_jitter_amount"), default_config.antihit.peek_jitter_amount);

			save_bool(antihit, CXOR("flick_enable"), default_config.antihit.flick_enable);
			save_int(antihit, CXOR("flick_interval"), default_config.antihit.flick_interval);
			save_int(antihit, CXOR("flick_duration"), default_config.antihit.flick_duration);
			save_int(antihit, CXOR("flick_yaw"), default_config.antihit.flick_yaw);

			save_int(antihit, CXOR("air_pitch"), default_config.antihit.air_pitch);
			save_int(antihit, CXOR("air_yaw"), default_config.antihit.air_yaw);
			save_int(antihit, CXOR("air_yaw_add"), default_config.antihit.air_yaw_add);
			save_int(antihit, CXOR("air_jitter_mode"), default_config.antihit.air_jitter_mode);
			save_int(antihit, CXOR("air_jitter_range"), default_config.antihit.air_jitter_range);
			save_int(antihit, CXOR("air_desync_left"), default_config.antihit.air_desync_left);
			save_int(antihit, CXOR("air_desync_right"), default_config.antihit.air_desync_right);
			save_int(antihit, CXOR("velocity_yaw_bias_amount"), default_config.antihit.velocity_yaw_bias_amount);
			save_int(antihit, CXOR("adaptive_jitter_scale"), default_config.antihit.adaptive_jitter_scale);
			save_int(antihit, CXOR("landing_pitch_ticks"), default_config.antihit.landing_pitch_ticks);
			save_int(antihit, CXOR("landing_pitch_value"), default_config.antihit.landing_pitch_value);

			save_uint(antihit, CXOR("fakelag_conditions"), default_config.antihit.fakelag_conditions);
			save_int(antihit, CXOR("manual_jitter"), default_config.antihit.manual_jitter);

			save_int(antihit, CXOR("freestand_mode"), default_config.antihit.freestand_mode);
			save_int(antihit, CXOR("freestand_offset"), default_config.antihit.freestand_offset);
			save_int(antihit, CXOR("freestand_hold"), default_config.antihit.freestand_hold);
			save_int(antihit, CXOR("freestand_smooth"), default_config.antihit.freestand_smooth);
			save_int(antihit, CXOR("freestand_jitter"), default_config.antihit.freestand_jitter);
			save_int(antihit, CXOR("freestand_dist"), default_config.antihit.freestand_dist);
			save_int(antihit, CXOR("freestand_fov"), default_config.antihit.freestand_fov);
			save_int(antihit, CXOR("freestand_custom"), default_config.antihit.freestand_custom);
			save_bool(antihit, CXOR("freestand_avoid_flick"), default_config.antihit.freestand_avoid_flick);

			save_int(antihit, CXOR("custom_pitch"), default_config.antihit.custom_pitch);
			save_int(antihit, CXOR("random_pitch"), default_config.antihit.random_pitch);
			save_int(antihit, CXOR("random_pitch2"), default_config.antihit.random_pitch2);
			save_int(antihit, CXOR("switch_pitch"), default_config.antihit.switch_pitch);
			save_int(antihit, CXOR("switch_pitch2"), default_config.antihit.switch_pitch2);
			save_int(antihit, CXOR("way_pitch"), default_config.antihit.way_pitch);
			save_int(antihit, CXOR("way_pitch2"), default_config.antihit.way_pitch2);
		}
		...
		auto& visuals = json_obj[CXOR("visuals")];
		{
			...
			save_bool(visuals[CXOR("eventlog")], CXOR("kill_say_enable"), default_config.visuals.eventlog.kill_say_enable);
			save_int(visuals[CXOR("eventlog")], CXOR("kill_say_style"), default_config.visuals.eventlog.kill_say_style);
		...
		auto& misc = json_obj[CXOR("misc")];
		{
			...
			save_bool(misc, CXOR("retrack_peek_halo"), default_config.misc.retrack_peek_halo);
			...
			save_int(misc[CXOR("sunset_angle")], CXOR("z"), default_config.misc.sunset_angle.z);
			save_clr(misc, CXOR("retrack_peek_halo_clr"), &default_config.misc.retrack_peek_halo_clr);
		...
		for (int i = 0; i < (int)str.size(); ++i)
			str[i] ^= static_cast<char>(CFG_XOR_KEY & 0xFF);

		std::ofstream file_out(file, std::ios::binary);
		if (file_out.good())
			file_out.write(str.data(), static_cast<std::streamsize>(str.size()));
		file_out.close();
	}
	...
	void save(const std::string& config_name)
	{
		...
		auto& rage = json_obj[CXOR("rage")];
		{
			...
			save_bool(rage, CXOR("multipoint_debug"), g_cfg.rage.multipoint_debug);
			save_bool(rage, CXOR("multipoint_advanced"), g_cfg.rage.multipoint_advanced);
			save_bool(rage, CXOR("debug_hitscan"), g_cfg.rage.debug_hitscan);
			save_bool(rage, CXOR("debug_log"), g_cfg.rage.debug_log);
			save_bool(rage, CXOR("backtrack"), g_cfg.rage.backtrack);
			save_int(rage, CXOR("backtrack_ms"), g_cfg.rage.backtrack_ms);
			save_bool(rage, CXOR("extrapolation"), g_cfg.rage.extrapolation);
			save_int(rage, CXOR("extrapolation_ticks"), g_cfg.rage.extrapolation_ticks);
			save_int(rage, CXOR("interpolation_ticks"), g_cfg.rage.interpolation_ticks);
			save_bool(rage, CXOR("baim_on_jitter"), g_cfg.rage.baim_on_jitter);
			save_bool(rage, CXOR("auto_delay"), g_cfg.rage.auto_delay);
			save_int(rage, CXOR("auto_delay_max"), g_cfg.rage.auto_delay_max);
			//save_int(rage, CXOR("defensive_mode"), g_cfg.rage.defensive_mode);
		...
		auto& antihit = json_obj[CXOR("antihit")];
		{
			...
			save_bool(antihit, CXOR("antiam"), g_cfg.antihit.antiam);
			save_int(antihit, CXOR("slow_walk_speed"), g_cfg.antihit.slow_walk_speed);
			save_bool(antihit, CXOR("at_targets"), g_cfg.antihit.at_targets);

			save_bool(antihit, CXOR("at_targets_enabled"), g_cfg.antihit.at_targets_enabled);
			save_int(antihit, CXOR("at_targets_offset"), g_cfg.antihit.at_targets_offset);
			save_int(antihit, CXOR("manual_offset"), g_cfg.antihit.manual_offset);
			...
			save_uint(antihit, CXOR("def_aa_conditions"), g_cfg.antihit.def_aa_conditions);
			save_bool(antihit, CXOR("def_aa_enable"), g_cfg.antihit.def_aa_enable);
			save_bool(antihit, CXOR("def_aa_dt_active"), g_cfg.antihit.def_aa_dt_active);
			save_int(antihit, CXOR("def_aa_dt_mode"), g_cfg.antihit.def_aa_dt_mode);
			save_int(antihit, CXOR("def_aa_yaw_preset"), g_cfg.antihit.def_aa_yaw_preset);
			save_int(antihit, CXOR("def_aa_phase_count"), g_cfg.antihit.def_aa_phase_count);
			for (int p = 0; p < 4; ++p)
			{
				auto phase_key = std::string("def_aa_phase_") + std::to_string(p);
				save_int(antihit[phase_key], CXOR("duration"), g_cfg.antihit.def_aa_phases[p].duration);
				save_int(antihit[phase_key], CXOR("yaw_mode"), g_cfg.antihit.def_aa_phases[p].yaw_mode);
				save_int(antihit[phase_key], CXOR("yaw_angle"), g_cfg.antihit.def_aa_phases[p].yaw_angle);
				save_int(antihit[phase_key], CXOR("pitch_mode"), g_cfg.antihit.def_aa_phases[p].pitch_mode);
				save_int(antihit[phase_key], CXOR("pitch_angle"), g_cfg.antihit.def_aa_phases[p].pitch_angle);

				save_int(antihit[phase_key], CXOR("pitch_angle2"), g_cfg.antihit.def_aa_phases[p].pitch_angle2);
				save_int(antihit[phase_key], CXOR("spin_rpm"), g_cfg.antihit.def_aa_phases[p].spin_rpm);
				save_int(antihit[phase_key], CXOR("yaw_rpm"), g_cfg.antihit.def_aa_phases[p].yaw_rpm);
				save_bool(antihit[phase_key], CXOR("spin_clockwise"), g_cfg.antihit.def_aa_phases[p].spin_clockwise);
				save_int(antihit[phase_key], CXOR("spin_inverter_mode"), g_cfg.antihit.def_aa_phases[p].spin_inverter_mode);
			}

			save_int(antihit, CXOR("def_aa_rotate_ticks"), g_cfg.antihit.def_aa_rotate_ticks);
			save_int(antihit, CXOR("def_aa_yaw_left"), g_cfg.antihit.def_aa_yaw_left);
			save_int(antihit, CXOR("def_aa_yaw_right"), g_cfg.antihit.def_aa_yaw_right);
			save_int(antihit, CXOR("def_aa_yaw_back"), g_cfg.antihit.def_aa_yaw_back);
			save_bool(antihit, CXOR("def_aa_random_yaw"), g_cfg.antihit.def_aa_random_yaw);
			save_int(antihit, CXOR("def_aa_random_yaw_min"), g_cfg.antihit.def_aa_random_yaw_min);
			save_int(antihit, CXOR("def_aa_random_yaw_max"), g_cfg.antihit.def_aa_random_yaw_max);
			save_int(antihit, CXOR("def_aa_pitch"), g_cfg.antihit.def_aa_pitch);
			save_bool(antihit, CXOR("def_aa_random_pitch"), g_cfg.antihit.def_aa_random_pitch);
			save_int(antihit, CXOR("def_aa_random_pitch_min"), g_cfg.antihit.def_aa_random_pitch_min);
			save_int(antihit, CXOR("def_aa_random_pitch_max"), g_cfg.antihit.def_aa_random_pitch_max);
			...
			save_bool(antihit, CXOR("peek_pitch_enable"), g_cfg.antihit.peek_pitch_enable);
			save_bool(antihit, CXOR("air_override"), g_cfg.antihit.air_override);
			save_bool(antihit, CXOR("velocity_yaw_bias"), g_cfg.antihit.velocity_yaw_bias);
			save_bool(antihit, CXOR("adaptive_jitter"), g_cfg.antihit.adaptive_jitter);
			save_bool(antihit, CXOR("landing_pitch"), g_cfg.antihit.landing_pitch);

			save_int(antihit, CXOR("pitch"), g_cfg.antihit.pitch);
			save_int(antihit, CXOR("yaw"), g_cfg.antihit.yaw);
			save_int(antihit, CXOR("custom_yaw"), g_cfg.antihit.custom_yaw);
			save_int(antihit, CXOR("ticks_defensive"), g_cfg.antihit.ticks_defensive);

			save_int(antihit, CXOR("def_aa_random_factor"), g_cfg.antihit.def_aa_random_factor);
			...
			save_int(antihit, CXOR("peek_pitch_phase"), g_cfg.antihit.peek_pitch_phase);
			save_int(antihit, CXOR("peek_pitch_mode"), g_cfg.antihit.peek_pitch_mode);
			save_int(antihit, CXOR("peek_custom_pitch"), g_cfg.antihit.peek_custom_pitch);
			save_int(antihit, CXOR("peek_jitter_mode"), g_cfg.antihit.peek_jitter_mode);
			save_int(antihit, CXOR("peek_jitter_amount"), g_cfg.antihit.peek_jitter_amount);

			save_bool(antihit, CXOR("flick_enable"), g_cfg.antihit.flick_enable);
			save_int(antihit, CXOR("flick_interval"), g_cfg.antihit.flick_interval);
			save_int(antihit, CXOR("flick_duration"), g_cfg.antihit.flick_duration);
			save_int(antihit, CXOR("flick_yaw"), g_cfg.antihit.flick_yaw);

			save_int(antihit, CXOR("air_pitch"), g_cfg.antihit.air_pitch);
			save_int(antihit, CXOR("air_yaw"), g_cfg.antihit.air_yaw);
			save_int(antihit, CXOR("air_yaw_add"), g_cfg.antihit.air_yaw_add);
			save_int(antihit, CXOR("air_jitter_mode"), g_cfg.antihit.air_jitter_mode);
			save_int(antihit, CXOR("air_jitter_range"), g_cfg.antihit.air_jitter_range);
			save_int(antihit, CXOR("air_desync_left"), g_cfg.antihit.air_desync_left);
			save_int(antihit, CXOR("air_desync_right"), g_cfg.antihit.air_desync_right);
			save_int(antihit, CXOR("velocity_yaw_bias_amount"), g_cfg.antihit.velocity_yaw_bias_amount);
			save_int(antihit, CXOR("adaptive_jitter_scale"), g_cfg.antihit.adaptive_jitter_scale);
			save_int(antihit, CXOR("landing_pitch_ticks"), g_cfg.antihit.landing_pitch_ticks);
			save_int(antihit, CXOR("landing_pitch_value"), g_cfg.antihit.landing_pitch_value);

			save_uint(antihit, CXOR("fakelag_conditions"), g_cfg.antihit.fakelag_conditions);
			save_int(antihit, CXOR("manual_jitter"), g_cfg.antihit.manual_jitter);

			save_int(antihit, CXOR("freestand_mode"), g_cfg.antihit.freestand_mode);
			save_int(antihit, CXOR("freestand_offset"), g_cfg.antihit.freestand_offset);
			save_int(antihit, CXOR("freestand_hold"), g_cfg.antihit.freestand_hold);
			save_int(antihit, CXOR("freestand_smooth"), g_cfg.antihit.freestand_smooth);
			save_int(antihit, CXOR("freestand_jitter"), g_cfg.antihit.freestand_jitter);
			save_int(antihit, CXOR("freestand_dist"), g_cfg.antihit.freestand_dist);
			save_int(antihit, CXOR("freestand_fov"), g_cfg.antihit.freestand_fov);
			save_int(antihit, CXOR("freestand_custom"), g_cfg.antihit.freestand_custom);
			save_bool(antihit, CXOR("freestand_avoid_flick"), g_cfg.antihit.freestand_avoid_flick);
		...
		auto& visuals = json_obj[CXOR("visuals")];
		{
			...
			save_bool(visuals[CXOR("eventlog")], CXOR("kill_say_enable"), g_cfg.visuals.eventlog.kill_say_enable);
			save_int(visuals[CXOR("eventlog")], CXOR("kill_say_style"), g_cfg.visuals.eventlog.kill_say_style);
		...
		auto& misc = json_obj[CXOR("misc")];
		{
			...
			save_bool(misc, CXOR("retrack_peek_halo"), g_cfg.misc.retrack_peek_halo);
			...
			save_int(misc[CXOR("sunset_angle")], CXOR("z"), g_cfg.misc.sunset_angle.z);

			save_clr(misc, CXOR("autopeek_clr"), &g_cfg.misc.autopeek_clr);
			save_clr(misc, CXOR("autopeek_clr_back"), &g_cfg.misc.autopeek_clr_back);
			save_clr(misc, CXOR("retrack_peek_halo_clr"), &g_cfg.misc.retrack_peek_halo_clr);
	...
	void load(const std::string& config_name)
	{
		...
		auto& rage = json_obj[CXOR("rage")];
		{
			...
			load_bool(rage, CXOR("multipoint_debug"), g_cfg.rage.multipoint_debug);
			load_bool(rage, CXOR("multipoint_advanced"), g_cfg.rage.multipoint_advanced);
			load_bool(rage, CXOR("debug_hitscan"), g_cfg.rage.debug_hitscan);
			load_bool(rage, CXOR("debug_log"), g_cfg.rage.debug_log);
			load_bool(rage, CXOR("backtrack"), g_cfg.rage.backtrack);
			load_int(rage, CXOR("backtrack_ms"), g_cfg.rage.backtrack_ms);
			load_bool(rage, CXOR("extrapolation"), g_cfg.rage.extrapolation);
			load_int(rage, CXOR("extrapolation_ticks"), g_cfg.rage.extrapolation_ticks);
			load_int(rage, CXOR("interpolation_ticks"), g_cfg.rage.interpolation_ticks);
			load_bool(rage, CXOR("baim_on_jitter"), g_cfg.rage.baim_on_jitter);
			load_bool(rage, CXOR("auto_delay"), g_cfg.rage.auto_delay);
			load_int(rage, CXOR("auto_delay_max"), g_cfg.rage.auto_delay_max);
			// Advanced multipoint toggle removed: always on.
			g_cfg.rage.multipoint_advanced = true;
			//load_int(rage, CXOR("defensive_mode"), g_cfg.rage.defensive_mode);
		...
		auto& antihit = json_obj[CXOR("antihit")];
		{
			...
			load_bool(antihit, CXOR("antiam"), g_cfg.antihit.antiam);
			load_int(antihit, CXOR("slow_walk_speed"), g_cfg.antihit.slow_walk_speed);
			load_bool(antihit, CXOR("at_targets"), g_cfg.antihit.at_targets);

			load_bool(antihit, CXOR("at_targets_enabled"), g_cfg.antihit.at_targets_enabled);
			load_int(antihit, CXOR("at_targets_offset"), g_cfg.antihit.at_targets_offset);
			load_int(antihit, CXOR("manual_offset"), g_cfg.antihit.manual_offset);

			load_int(antihit, CXOR("def_aa_mode"), g_cfg.antihit.def_aa_mode);
			g_cfg.antihit.def_aa_mode = std::clamp(g_cfg.antihit.def_aa_mode, 0, 1);
			load_bool(antihit, CXOR("def_pitch"), g_cfg.antihit.def_pitch);
			load_bool(antihit, CXOR("def_yaw"), g_cfg.antihit.def_yaw);
			load_bool(antihit, CXOR("def_aa_enable"), g_cfg.antihit.def_aa_enable);
			load_bool(antihit, CXOR("def_aa_dt_active"), g_cfg.antihit.def_aa_dt_active);
			load_int(antihit, CXOR("def_aa_dt_mode"), g_cfg.antihit.def_aa_dt_mode);
			if (antihit[CXOR("def_aa_dt_mode")].empty() && g_cfg.antihit.def_aa_dt_active)
				g_cfg.antihit.def_aa_dt_mode = 3;
			g_cfg.antihit.def_aa_dt_mode = std::clamp(g_cfg.antihit.def_aa_dt_mode, 0, 4);
			const bool missing_def_aa_conditions = antihit[CXOR("def_aa_conditions")].empty();
			load_uint(antihit, CXOR("def_aa_conditions"), g_cfg.antihit.def_aa_conditions);
			if (missing_def_aa_conditions)
			{
				g_cfg.antihit.def_aa_conditions = g_cfg.antihit.def_aa_mode == 1
					? def_aa_cond_tickbase_choke
					: def_aa_cond_tickbase_trigger;

				if (g_cfg.antihit.def_aa_dt_active)
				{
					switch (g_cfg.antihit.def_aa_dt_mode)
					{
					case 0: g_cfg.antihit.def_aa_conditions |= def_aa_cond_on_shot; break;
					case 1: g_cfg.antihit.def_aa_conditions |= def_aa_cond_peek; break;
					case 2: g_cfg.antihit.def_aa_conditions |= def_aa_cond_in_air; break;
					case 3: g_cfg.antihit.def_aa_conditions |= def_aa_cond_always; break;
					case 4: g_cfg.antihit.def_aa_conditions |= def_aa_cond_in_crouch; break;
					default: break;
					}
				}
			}
			g_cfg.antihit.def_aa_conditions &= def_aa_cond_all;
			load_int(antihit, CXOR("def_aa_yaw_preset"), g_cfg.antihit.def_aa_yaw_preset);
			g_cfg.antihit.def_aa_yaw_preset = std::clamp(g_cfg.antihit.def_aa_yaw_preset, 0, 4);
			load_int(antihit, CXOR("def_aa_phase_count"), g_cfg.antihit.def_aa_phase_count);
			for (int p = 0; p < 4; ++p)
			{
				auto phase_key = std::string("def_aa_phase_") + std::to_string(p);
				load_int(antihit[phase_key], CXOR("duration"), g_cfg.antihit.def_aa_phases[p].duration);
				load_int(antihit[phase_key], CXOR("yaw_mode"), g_cfg.antihit.def_aa_phases[p].yaw_mode);
				load_int(antihit[phase_key], CXOR("yaw_angle"), g_cfg.antihit.def_aa_phases[p].yaw_angle);
				load_int(antihit[phase_key], CXOR("pitch_mode"), g_cfg.antihit.def_aa_phases[p].pitch_mode);
				load_int(antihit[phase_key], CXOR("pitch_angle"), g_cfg.antihit.def_aa_phases[p].pitch_angle);

				load_int(antihit[phase_key], CXOR("pitch_angle2"), g_cfg.antihit.def_aa_phases[p].pitch_angle2);
				load_int(antihit[phase_key], CXOR("spin_rpm"), g_cfg.antihit.def_aa_phases[p].spin_rpm);
				load_int(antihit[phase_key], CXOR("yaw_rpm"), g_cfg.antihit.def_aa_phases[p].yaw_rpm);
				load_bool(antihit[phase_key], CXOR("spin_clockwise"), g_cfg.antihit.def_aa_phases[p].spin_clockwise);
				load_int(antihit[phase_key], CXOR("spin_inverter_mode"), g_cfg.antihit.def_aa_phases[p].spin_inverter_mode);

				g_cfg.antihit.def_aa_phases[p].duration = std::clamp(g_cfg.antihit.def_aa_phases[p].duration, 1, 32);
				g_cfg.antihit.def_aa_phases[p].yaw_mode = std::clamp(g_cfg.antihit.def_aa_phases[p].yaw_mode, 0, 9);
				g_cfg.antihit.def_aa_phases[p].yaw_angle = std::clamp(g_cfg.antihit.def_aa_phases[p].yaw_angle, -180, 180);
				g_cfg.antihit.def_aa_phases[p].pitch_mode = std::clamp(g_cfg.antihit.def_aa_phases[p].pitch_mode, 0, 8);
				g_cfg.antihit.def_aa_phases[p].pitch_angle = std::clamp(g_cfg.antihit.def_aa_phases[p].pitch_angle, -89, 89);
				g_cfg.antihit.def_aa_phases[p].pitch_angle2 = std::clamp(g_cfg.antihit.def_aa_phases[p].pitch_angle2, -89, 89);
				g_cfg.antihit.def_aa_phases[p].spin_rpm = std::clamp(g_cfg.antihit.def_aa_phases[p].spin_rpm, 1, 720);
				g_cfg.antihit.def_aa_phases[p].yaw_rpm = std::clamp(g_cfg.antihit.def_aa_phases[p].yaw_rpm, 1, 500);
				g_cfg.antihit.def_aa_phases[p].spin_inverter_mode = std::clamp(g_cfg.antihit.def_aa_phases[p].spin_inverter_mode, 0, 3);
			}
			g_cfg.antihit.def_aa_phase_count = std::clamp(g_cfg.antihit.def_aa_phase_count, 1, configs_t::anti_hit_t::DEF_AA_MAX_PHASES);
			load_int(antihit, CXOR("def_aa_air_minimum_ticks"), g_cfg.antihit.def_aa_air_minimum_ticks);
			load_int(antihit, CXOR("def_aa_rotate_ticks"), g_cfg.antihit.def_aa_rotate_ticks);
			load_int(antihit, CXOR("def_aa_yaw_left"), g_cfg.antihit.def_aa_yaw_left);
			load_int(antihit, CXOR("def_aa_yaw_right"), g_cfg.antihit.def_aa_yaw_right);
			load_int(antihit, CXOR("def_aa_yaw_back"), g_cfg.antihit.def_aa_yaw_back);
			load_bool(antihit, CXOR("def_aa_random_yaw"), g_cfg.antihit.def_aa_random_yaw);
			load_int(antihit, CXOR("def_aa_random_yaw_min"), g_cfg.antihit.def_aa_random_yaw_min);
			load_int(antihit, CXOR("def_aa_random_yaw_max"), g_cfg.antihit.def_aa_random_yaw_max);
			load_int(antihit, CXOR("def_aa_pitch"), g_cfg.antihit.def_aa_pitch);
			load_bool(antihit, CXOR("def_aa_random_pitch"), g_cfg.antihit.def_aa_random_pitch);
			load_int(antihit, CXOR("def_aa_random_pitch_min"), g_cfg.antihit.def_aa_random_pitch_min);
			load_int(antihit, CXOR("def_aa_random_pitch_max"), g_cfg.antihit.def_aa_random_pitch_max);
			...
			load_bool(antihit, CXOR("peek_pitch_enable"), g_cfg.antihit.peek_pitch_enable);
			load_bool(antihit, CXOR("air_override"), g_cfg.antihit.air_override);
			load_bool(antihit, CXOR("velocity_yaw_bias"), g_cfg.antihit.velocity_yaw_bias);
			load_bool(antihit, CXOR("adaptive_jitter"), g_cfg.antihit.adaptive_jitter);
			load_bool(antihit, CXOR("landing_pitch"), g_cfg.antihit.landing_pitch);

			load_int(antihit, CXOR("pitch"), g_cfg.antihit.pitch);
			load_int(antihit, CXOR("yaw"), g_cfg.antihit.yaw);
			if (!antihit[CXOR("custom_yaw")].empty())
				load_int(antihit, CXOR("custom_yaw"), g_cfg.antihit.custom_yaw);
			else
				load_int(antihit, CXOR("way_pitch2"), g_cfg.antihit.custom_yaw);
			load_int(antihit, CXOR("ticks_defensive"), g_cfg.antihit.ticks_defensive);

			load_int(antihit, CXOR("def_aa_random_factor"), g_cfg.antihit.def_aa_random_factor);
			g_cfg.antihit.def_aa_random_factor = std::clamp(g_cfg.antihit.def_aa_random_factor, 1, 10);

			load_int(antihit, CXOR("yaw_add"), g_cfg.antihit.yaw_add);

			load_int(antihit, CXOR("spin_speed"), g_cfg.antihit.spin_speed);
			load_bool(antihit, CXOR("spin_clamp"), g_cfg.antihit.spin_clamp);
			g_cfg.antihit.spin_speed = std::clamp(g_cfg.antihit.spin_speed, 1, 60);
			...
			load_int(antihit, CXOR("peek_pitch_phase"), g_cfg.antihit.peek_pitch_phase);
			load_int(antihit, CXOR("peek_pitch_mode"), g_cfg.antihit.peek_pitch_mode);
			load_int(antihit, CXOR("peek_custom_pitch"), g_cfg.antihit.peek_custom_pitch);
			load_int(antihit, CXOR("peek_jitter_mode"), g_cfg.antihit.peek_jitter_mode);
			load_int(antihit, CXOR("peek_jitter_amount"), g_cfg.antihit.peek_jitter_amount);

			load_bool(antihit, CXOR("flick_enable"), g_cfg.antihit.flick_enable);
			load_int(antihit, CXOR("flick_interval"), g_cfg.antihit.flick_interval);
			load_int(antihit, CXOR("flick_duration"), g_cfg.antihit.flick_duration);
			load_int(antihit, CXOR("flick_yaw"), g_cfg.antihit.flick_yaw);

			load_int(antihit, CXOR("air_pitch"), g_cfg.antihit.air_pitch);
			load_int(antihit, CXOR("air_yaw"), g_cfg.antihit.air_yaw);
			load_int(antihit, CXOR("air_yaw_add"), g_cfg.antihit.air_yaw_add);
			load_int(antihit, CXOR("air_jitter_mode"), g_cfg.antihit.air_jitter_mode);
			load_int(antihit, CXOR("air_jitter_range"), g_cfg.antihit.air_jitter_range);
			load_int(antihit, CXOR("air_desync_left"), g_cfg.antihit.air_desync_left);
			load_int(antihit, CXOR("air_desync_right"), g_cfg.antihit.air_desync_right);
			load_int(antihit, CXOR("velocity_yaw_bias_amount"), g_cfg.antihit.velocity_yaw_bias_amount);
			load_int(antihit, CXOR("adaptive_jitter_scale"), g_cfg.antihit.adaptive_jitter_scale);
			load_int(antihit, CXOR("landing_pitch_ticks"), g_cfg.antihit.landing_pitch_ticks);
			load_int(antihit, CXOR("landing_pitch_value"), g_cfg.antihit.landing_pitch_value);

			load_int(antihit, CXOR("manual_jitter"), g_cfg.antihit.manual_jitter);

			load_int(antihit, CXOR("freestand_mode"), g_cfg.antihit.freestand_mode);
			load_int(antihit, CXOR("freestand_offset"), g_cfg.antihit.freestand_offset);
			load_int(antihit, CXOR("freestand_hold"), g_cfg.antihit.freestand_hold);
			load_int(antihit, CXOR("freestand_smooth"), g_cfg.antihit.freestand_smooth);
			load_int(antihit, CXOR("freestand_jitter"), g_cfg.antihit.freestand_jitter);
			load_int(antihit, CXOR("freestand_dist"), g_cfg.antihit.freestand_dist);
			load_int(antihit, CXOR("freestand_fov"), g_cfg.antihit.freestand_fov);
			load_int(antihit, CXOR("freestand_custom"), g_cfg.antihit.freestand_custom);
			load_bool(antihit, CXOR("freestand_avoid_flick"), g_cfg.antihit.freestand_avoid_flick);
		...
		auto& visuals = json_obj[CXOR("visuals")];
		{
			...
			load_bool(visuals[CXOR("eventlog")], CXOR("kill_say_enable"), g_cfg.visuals.eventlog.kill_say_enable);
			load_int(visuals[CXOR("eventlog")], CXOR("kill_say_style"), g_cfg.visuals.eventlog.kill_say_style);
			if (g_cfg.visuals.eventlog.kill_say_style < 0 || g_cfg.visuals.eventlog.kill_say_style >= 5)
		...
		auto& misc = json_obj[CXOR("misc")];
		{
			...
			load_bool(misc, CXOR("retrack_peek_halo"), g_cfg.misc.retrack_peek_halo);
			...
			load_int(misc[CXOR("sunset_angle")], CXOR("z"), g_cfg.misc.sunset_angle.z);

			load_clr(misc, CXOR("retrack_peek_halo_clr"), g_cfg.misc.retrack_peek_halo_clr);
