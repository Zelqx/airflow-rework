...
#endif
{
	...
	// Backtrack diagnostics: how many records this player has, how many pass
	// the validity window and how many are flagged out-of-order (shifting).
	int dbg_records = 0, dbg_valid = 0, dbg_shifting = 0;
	for (auto& i : anim->records)
	{
		...
		// matrix_orig must be clamped with the RECORD's eye, not the live eye.
		ptr->clamp_matrix(player, &i.matrix_orig, fresh_tick, rec_angles, clamp_info);
		...
		ptr->clamp_matrix(player, &i.matrix_left_extra, fresh_tick, rec_angles, clamp_info);
		ptr->build_roll_matrix(player, &i.matrix_left_extra, -1, fresh_tick, rec_angles, clamp_info);

		ptr->clamp_matrix(player, &i.matrix_right_extra, fresh_tick, rec_angles, clamp_info);
		ptr->build_roll_matrix(player, &i.matrix_right_extra, 1, fresh_tick, rec_angles, clamp_info);

		// Copy layers into ALL matrix variants
		std::memcpy(i.matrix_orig.layers, i.layers, sizeof(i.layers));
		std::memcpy(i.matrix_left.layers, i.layers, sizeof(i.layers));
		std::memcpy(i.matrix_right.layers, i.layers, sizeof(i.layers));
		std::memcpy(i.matrix_zero.layers, i.layers, sizeof(i.layers));
		std::memcpy(i.matrix_left_extra.layers, i.layers, sizeof(i.layers));
		std::memcpy(i.matrix_right_extra.layers, i.layers, sizeof(i.layers));
#endif
	}

	anim->dbg_records = dbg_records;
	anim->dbg_valid = dbg_valid;
	anim->dbg_shifting = dbg_shifting;
}
void c_lag_comp::update_tick_validation()
{
	if (!HACKS->in_game || !HACKS->local || !HACKS->client_state || HACKS->client_state->delta_tick == -1)
		return;
