...
struct esp_player_t
{
    ...
    float duck_amount{};
    float alpha = 1.f;
    float last_duck_amount = -1.f;
    int duck_direction = 0;
    int duck_direction_changes = 0;
    int fake_duck_ticks = 0;
    ...
    INLINE void update(c_cs_player* player)
    {
        ...
        maxs = player->bb_maxs();
        duck_amount = player->duck_amount();
        auto player_flags = player->flags();
        const float duck_delta = duck_amount - last_duck_amount;
        const int current_direction = std::fabs(duck_delta) > 0.035f ? (duck_delta > 0.f ? 1 : -1) : 0;

        if (current_direction != 0 && duck_direction != 0 && current_direction != duck_direction)
            duck_direction_changes = std::min(duck_direction_changes + 1, 4);
        else if (current_direction == 0)
            duck_direction_changes = std::max(duck_direction_changes - 1, 0);

        if (current_direction != 0)
            duck_direction = current_direction;

        if (player_flags.has(FL_ONGROUND) && player_flags.has(FL_DUCKING) && duck_direction_changes >= 2)
            fake_duck_ticks = 8;
        else if (fake_duck_ticks > 0)
            --fake_duck_ticks;

        fake_ducking = fake_duck_ticks > 0;
    ...
    INLINE void reset()
    {
        if (valid)
        {
            ...
            alpha = 1.f;
            duck_amount = 0.f;
            last_duck_amount = -1.f;
            duck_direction = 0;
            duck_direction_changes = 0;
            fake_duck_ticks = 0;
            fake_ducking = false;
           // dormant.reset();
            box.reset();
...
{
private:
	weapon_esp_t weapon_esp[2048]{};
    esp_player_t player_esp[128]{};
