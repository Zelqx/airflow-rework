...
int c_fake_lag::get_choke_amount()
{
	...
	// safer initialization for the timer used for in-air fluctuation
	static int timer{};
	if (timer == 0)
	...
	{
		if (std::abs(timer - HACKS->global_vars->tickcount) > max_choke)
		{
	...
	// smooth abrupt changes in choke to avoid one-frame 'hiccups'
	static int prev_choke = 0;
	int final_choke = temp_choke;
	if (prev_choke == 0)
		prev_choke = final_choke;

	// limit change per tick to 1 so local movement/prediction doesn't jump
	if (std::abs(final_choke - prev_choke) > 1)
		final_choke = prev_choke + ((final_choke > prev_choke) ? 1 : -1);
...
#else
	// removed the periodic tickbase check ("% 100") which forced a send every 100 tickbase
	if (EXPLOITS->recharge.start || choke_amount == 0 || (shooting) && !ANTI_AIM->is_fake_ducking())
#endif
	{
		if (!*HACKS->send_packet)
			*HACKS->send_packet = true;
