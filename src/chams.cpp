...
bool c_chams::should_draw()
{
	...
	if (std::strstr(model->name, CXOR("models/player")))
	{
		...
		if (player == HACKS->local)
		{
			...
			const bool fake_on = g_cfg.antihit.desync && g_cfg.visuals.chams[c_fake].enable
				&& local_anim && local_anim->real_valid;

			// Draw the fake body first as an overlay (ignorez) so the real body
			// can layer on top of it and never disappears.
			if (fake_on)
			{
				static matrix3x4_t fake_world[128]{};
				math::memcpy_sse(fake_world, local_anim->matrix, sizeof(fake_world));
				math::change_bones_position(fake_world, 128, {}, HACKS->local->get_abs_origin());

				this->draw_model(g_cfg.visuals.chams[c_fake], fake_world, alpha, true);
			}

			// Real body chams (engine bone cache = real_matrix). If the real
			// chams is off we return false so the game still draws the normal
			// real model - the fake remains as an overlay.
			const bool drew_real = this->draw_model(g_cfg.visuals.chams[c_local], nullptr, alpha);

			if (rage_log::enabled())
				rage_log::line("[CHAMS] local real_on=%d fake_on=%d drew_real=%d real_valid=%d foot=%.1f",
					g_cfg.visuals.chams[c_local].enable, fake_on, drew_real,
					local_anim ? local_anim->real_valid : 0,
					local_anim ? local_anim->foot_yaw : 0.f);
...
void c_chams::add_shot_record(c_cs_player* player, matrix3x4_t* matrix)
{
	...
	// Copy exactly 128 bones. The old sizeof(shot.bones) copied 256 bones from
	// a 128-bone source matrix; for extrapolated/predicted matrices (last member
	// of anim_record_t) that read ~6 KB past the allocation and crashed on fire.
	math::memcpy_sse(shot.bones, matrix, sizeof(matrix3x4_t) * 128);
...
void c_chams::on_draw_model_execute(dme_original original, void* ecx, void* edx, void* ctx, const draw_model_state_t& state, const model_render_info_t& info, matrix3x4_t* bone_to_world)
{
	...
	{
		if (rage_log::enabled() && HACKS->local && info.entity_index == HACKS->local->index())
			rage_log::line("[CHAMS] local engine draw (should_draw=0) delta=%d",
				HACKS->client_state ? HACKS->client_state->delta_tick : 0);
