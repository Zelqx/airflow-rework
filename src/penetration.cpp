...
namespace penetration
{
	INLINE void scale_damage(const int hitgroup, c_cs_player* entity, const float weapon_armor_ratio, const float weapon_head_shot_multiplier, float& damage)
	{
		...
		// CS:GO OnTakeDamage: heavy armor scales the incoming damage by 0.85
		// before the armor math (cs_player.cpp:4890-4895). This was missing.
		if (has_heavy_armor)
			damage *= 0.85f;
		...
		if (entity->is_armored(hitgroup))
		{
			...
			// CS:GO cs_player.cpp:4981-4986: heavy armor -> armor ratio halves
			// again and BOTH bonuses become 0.33. The old 0.25/0.20 values were
			// invented and made heavy-armor min-damage wrong.
	...
	bullet_t simulate(c_cs_player* shooter, c_cs_player* target, vec3_t source, const vec3_t& dest, bool ignore_damage, bool simple)
	{
		...
		while (current_damage > 0.f)
		{
			...
			// Damage falloff applies per traversed segment, not to the cumulative
			// distance over and over (the old code re-scaled already-scaled damage).
			const float segment_distance = enter_trace.fraction * max_distance;
			current_distance += segment_distance;
			current_damage *= std::pow(weapon_info->range_modifier, segment_distance / 500.f);

			// CS:GO only zeroes the penetration count here and STILL applies
			// damage to the current hit (cs_player_shared.cpp:1706-1721, then
			// 1809-1822). The old `break` skipped the target damage entirely, so
			// any shot past 3000u or through a <0.1 surface dealt 0 damage.
			if (current_distance > 3000.f || enter_penetration_modifier < 0.1f)
				out.penetration_count = 0;
