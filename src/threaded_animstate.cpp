...
static std::mutex animstate_mutex;
...
void c_threaded_animstate::setup_lean(c_animation_state* state, float curtime) {
	...
	*reinterpret_cast<vec3_t*>(reinterpret_cast<uintptr_t> (state) + 0x174) = math::approach(*reinterpret_cast<vec3_t*>(reinterpret_cast<uintptr_t> (state) + 0x168), *reinterpret_cast<vec3_t*>(reinterpret_cast<uintptr_t> (state) + 0x174), flInterval * 800.0f);
...
void c_threaded_animstate::setup_aim_matrix(c_animation_state* state, float curtime)
{
	...
	// Stale state->player (entity freed/reused) is the recurring wild-EIP crash;
	// this function dereferences it through the model/sequence/vtable paths.
	if (!player->is_alive()
		|| HACKS->entity_list->get_client_entity(player->index()) != player
...
void c_threaded_animstate::update(c_cs_player* player, c_animation_state* state, float yaw, float pitch, float curtime, int framecount) {
	std::lock_guard<std::mutex> lock(animstate_mutex);

	if (!player || !state) return; // safety check

	state->player = player;

	// Check cache_sequences pointer
	auto cache_seq_fn = offsets::cache_sequences.cast<bool(__thiscall*)(c_animation_state*)>();
	if (!cache_seq_fn || !cache_seq_fn(state))
		return;
	...
	// Hardening for the recurring thread-animstate wild-EIP crash
	// (c_threaded_animstate::update -> setup_*): the player/model/weapon can go
	// invalid between the worker beginning and here (round end, disconnect,
	// entity reuse). A bad indirect call inside the setup functions follows.
	// Bail cleanly instead of executing on a stale object.
	if (!player || !state || state->player != reinterpret_cast<void*>(player)
		|| !player->is_alive() || !player->get_studio_hdr()
		|| !HACKS->entity_list->get_client_entity_handle(player->active_weapon()))
		return;

	{
		...
		auto whole_body = &player->animlayers()[ANIMATION_LAYER_WHOLE_BODY];
