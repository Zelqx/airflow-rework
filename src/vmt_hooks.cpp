...
namespace hooks::vmt
{
	...
	bool __fastcall write_usercmd_to_delta_buffer(void* ecx, void* edx, int slot, void* buf, int from, int to, bool isnewcommand)
	{
		...
		//	return original(ecx, edx, slot, buf, from, to, isnewcommand);
		//
		// Hide shots (fake-command padding) must be allowed even though it does
		// not use the tickbase shift/recharge path.
		const bool shift_pending = EXPLOITS->enabled() && EXPLOITS->tick_to_shift != 0;
		const bool fake_pending = EXPLOITS->hide_shot_commands > 0;

		if (!shift_pending && !fake_pending)
			return original(ecx, edx, slot, buf, from, to, isnewcommand);
		...
		if (fake_pending && !shift_pending)
			return EXPLOITS->should_add_fake_commands(new_commands, backup_commands, ecx, edx, slot, buf, from, to);
	...
	bool __fastcall create_move(void* ecx, void* edx, float input_sample_time, c_user_cmd* cmd)
	{
		...
		if (netchannel)
		{
			...
			HACKS->incoming = netchannel->get_latency(FLOW_INCOMING);
		...
		HACKS->cmd = cmd;
		{
			...
			if (HACKS->local->is_alive())
			{
				...
				HACKS->engine->set_view_angles(MOVEMENT->get_camera_angle());
	...
	inline bool should_remove_ragdoll(const model_t* model, c_base_entity* ent) {
		if (!model) return false;
		// Check model name for ragdoll substring
		if (std::strstr(model->name, "ragdoll"))
			return true;
		// Optionally check client class name
		if (ent && ent->get_client_class() && std::strstr(ent->get_client_class()->network_name, "Ragdoll"))
			return true;
		return false;
	}
	void __fastcall draw_model_execute(void* ecx, void* edx, void* ctx, const draw_model_state_t& state, const model_render_info_t& info, matrix3x4_t* bone_to_world)
	{
		...
		c_base_entity* ent = reinterpret_cast<c_base_entity*>(HACKS->entity_list->get_client_entity(info.entity_index));

		// Ragdoll removal logic
		if ((g_cfg.misc.removals & REMOVAL_RAGDOLL) && should_remove_ragdoll(model, ent))
			return;
    ...
    static int __fastcall list_leaves_in_box(void* ecx, void* edx, const vec3_t& mins, const vec3_t& maxs, unsigned short* list, int list_max)
    {
