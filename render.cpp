...
private:
	...
	void _begin() noexcept
	{
		...
		copy_back_buffer_to_texture(blur_texture1, D3DTEXF_LINEAR); // Use linear filtering for upsampling
	...
	{
		// Only create textures/shaders if needed (already done, but keep!)
		create_textures();
		create_shaders();
		...
		// Reduce blur passes from 8 to 2 (horizontal + vertical)
...
// Fix for C6385: Reading invalid data from 'world_to_screen.mat'
// The matrix3x4_t only has 3 rows, so accessing mat[3][...] is out of bounds.
// To fix, add a helper to safely read the fourth row as a float[4] after the matrix, or use a properly sized matrix type.
// If the world-to-screen matrix is actually 4x4, change matrix3x4_t to matrix4x4_t, or reinterpret the memory safely.
...
bool c_render::screen_transform(const vec3_t& source, vec2_t& output)
{
    ...
    // Fix: Read the fourth row as a float[4] after the matrix
    const float* fourth_row = reinterpret_cast<const float*>(&world_to_screen.mat[0][0]) + 12;
    float w = fourth_row[0] * source.x + fourth_row[1] * source.y + fourth_row[2] * source.z + fourth_row[3];
