...
void c_movement::run()
{
	...
	camera_view_angle = base_view_angle;
...
// Swirling particle effect around autopeek circle
	if (g_cfg.misc.retrack_peek_halo && g_cfg.misc.retrack_peek && info.valid()) {
		auto halo_clr = g_cfg.misc.retrack_peek_halo_clr.base();
		...
		for (int i = 0; i < particle_count; ++i) {
			...
			// Swirl motion: orbit + radius wobble
			float swirl_angle = angle + time * 2.0f;        // spin around
			float radius = base_radius + std::sin(time * 2.0f + i) * 5.0f; // breathing radius

			vec3_t point3d = vec3_t(std::sin(swirl_angle), std::cos(swirl_angle), 0.f) * radius;
			vec2_t point2d;

			if (RENDER->world_to_screen(info.start_pos + point3d, point2d, true)) {
				// Alpha fade per particle for shimmer
				float alpha = 0.4f + 0.6f * std::sin(time * 5.0f + i);
				auto clr = halo_clr.new_alpha((int)(alpha * 255));

				// Draw small particle (dot)
				draw_list->AddCircleFilled(ImVec2(point2d.x, point2d.y), 2.0f, clr.as_imcolor());
			}
		}
	}
}