...
{
	// yes it's pasteed..
	static INLINE void accelerate(c_user_cmd& user_cmd, const vec3_t& wishdir, const float wishspeed, vec3_t& velocity, float acceleration)
	{
	...
	INLINE void full_walk_move(c_user_cmd& user_cmd, vec3_t& move, vec3_t& fwd, vec3_t& right, vec3_t& velocity)
	{
		...
		// ground_entity is a uint32 handle; "no ground" is the invalid handle
		// 0xFFFFFFFF, not INT_MAX. The old check was always true in air, so it
		// ran ground friction/walk_move mid-air and desynced from the server.
		if (unpredicted_vars->ground_entity != 0xFFFFFFFFu)
		{
			velocity.z = 0.f;
